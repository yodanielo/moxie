...
class Registro extends MantixBase
{
    ...
    {
        $r = new MantixGrid();
        $sql="select * from mox_chicas where seccion='Los nuestros'";
        $r->atributos=array("sql"=>$sql,"nropag"=>"20","ordenar"=>"id","url_form"=>"los_nuestros.php","url"=>"los_nuestros.php");
        $r->columnas=array(
    ...
    function pre_ins(){
        ...
        $this->datos->agregar("seccion","Los nuestros");
    }
    function pre_upd(){
        $this->datos->agregar("estado","1");
        $x=preg_replace("/height=\"[0-9]{3}\"/","height=\"200\"",$_POST["codigo"]);
