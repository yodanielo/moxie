...
class Registro extends MantixBase
{
    ...
    {
        $r = new MantixGrid();
        $sql="select * from mox_chicas where seccion='Todas las Chicas Moxie'";
        $r->atributos=array("sql"=>$sql,"nropag"=>"20","ordenar"=>"id","url_form"=>"todas_las_chicas_moxie.php","url"=>"todas_las_chicas_moxie.php");
        $r->columnas=array(
    ...
    function pre_ins(){
        ...
        $this->datos->agregar("seccion","Todas las Chicas Moxie");
    }
    function pre_upd(){
        $this->datos->agregar("estado","1");
        $x=preg_replace("/height=\"[0-9]{3}\"/","height=\"200\"",$_POST["codigo"]);
